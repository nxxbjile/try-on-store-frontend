...
export default function EditProductDialog({
  ...
  const [stock, setStock] = useState<{ size: string; quantity: number }[]>(
    initialProduct.stock?.map(s => ({ size: s.size, quantity: s.quantity })) || [{ size: "", quantity: 0 }]
  )
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      onEditProduct({ ...formData, _id: initialProduct._id })
      toast({
  ...
  const handleCategoryChange = (value: "shirt" | "t-shirt" | "pants") => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      >
        ...
        <div className="overflow-y-auto px-6 pb-6 pt-2" style={{ flex: 1, minHeight: 0 }}>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                {existingImages.map((url, idx) => (
                  ...
                  >
                    <span className="text-xs truncate max-w-[80px]">{url.split("/").pop()}</span>
                    <Button
                      type="button"
                      variant="ghost"
                      size="sm"
